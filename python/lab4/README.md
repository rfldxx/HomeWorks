...
Далее, пока $B[0] < A[0]*A[1]$ мы будем перетаскивать $B[0]$ в конец $A$. \
Затем удаляем $A[0]$ и $A[1]$, и записываем элемент $A[0]*A[1]$ в начало $B$.

* `fact3_l` - сравнение не самих (возможно очень длинных) чисел $B[0]$ и $A[0]*A[1]$, а просто количества знаков в них: $B[0].bit\_length()$. \
